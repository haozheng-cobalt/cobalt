...
SbSystemDeviceType GetDeviceType(std::string device_type_string) {
  for (auto& map : kDeviceTypeStrings) {
    if (!strcasecmp(map.device_type_string, device_type_string.c_str())) {
...
void InitializeUserAgentPlatformInfoFields(UserAgentPlatformInfo& info) {
  ...
  char value[starboard::kSystemPropertyMaxLength];
  bool result;
  ...
  // TODO(cobalt, b/374213479): should we get javascript_engine_version from
  // v8c?
  //   info.set_javascript_engine_version(
  //       script::GetJavaScriptEngineNameAndVersion());
  std::string javascript_engine_version = "v8/8.8.278.8-jit";
  info.set_javascript_engine_version(javascript_engine_version);

  // TODO(cobalt, b/374213479): should we get rasterizer_type from
  ...
  //           .rasterizer_name;
  std::string rasterizer_type = "gles";
...
// Evergreen info
  ...
  if (platform_info_extension) {
    if (platform_info_extension->version >= 1) {
      result = platform_info_extension->GetFirmwareVersionDetails(
          value, starboard::kSystemPropertyMaxLength);
      if (result) {
  ...
  // TODO(cobalt, b/374213479): are we going to retrieve build_id and
  // cobalt_version from cobalt/build/build_info.py?
  std::string cobalt_version = "26.lts.0";
  std::string build_id = "1234567";
  info.set_cobalt_version(cobalt_version);
  info.set_cobalt_build_version_number(build_id);

  // TODO(cobalt, b/374213479): how do we retrieve build type macro?
